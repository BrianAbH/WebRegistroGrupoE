...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $_SESSION['error'] = '<div class="alert alert-warning">' . $e->getMessage() . '</div>';
    } catch (ErrorBaseDatosException $e) {
        $_SESSION['error'] = '<div class="alert alert-danger">' . $e->getMessage() . '</div>';
    } catch (Exception $e) {
        $_SESSION['error'] = '<div class="alert alert-dark">Error inesperado: ' . $e->getMessage() . '</div>';
    }
