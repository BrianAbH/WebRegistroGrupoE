...
class Dispositivo {
    ...
    public function __construct() {
        $conn = new Database(); // Se instancia la conexión a la base de datos
    ...
    public function insertar($data) {
        extract($data);
        $stmt = $this->conexion->prepare("INSERT INTO datosdispositivos 
    ...
    public function actualizar($data) {
        extract($data);
        $stmt = $this->conexion->prepare("UPDATE datosdispositivos 
    ...
    public function eliminar($id) {
        $stmt = $this->conexion->prepare("UPDATE datosdispositivos SET Activo = 0 WHERE Id_Dispositivo = ?");
    ...
    public function obtenerDispositivos() {
        $stmt = $this->conexion->prepare("SELECT r.Id_Dispositivo, r.Id_Cliente, c.Nombre_Completo, c.Apellidos, 
    ...
    public function obtenerByCliente() {
        $stmt = $this->conexion->prepare("SELECT Id_Cliente, Nombre_Completo FROM datoscliente WHERE Activo = 1");
        $stmt->execute();
        return $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    }
}
