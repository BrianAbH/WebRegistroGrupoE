...
class Reparacion {
    private $conexion; // Propiedad que almacena la conexión
    ...
    public function insertar($data){
        extract($data); 
        $stmt = $this->conexion->prepare("INSERT INTO reparaciones 
    ...
    public function actualizar($data){ 
        ...
        $stmt = $this->conexion->prepare("UPDATE reparaciones 
    ...
    public function eliminar($id) {
        $stmt = $this->conexion->prepare("UPDATE reparaciones SET Activo = 0 WHERE Id_Reparacion = ?");
    ...
    public function obtenerReparacion() {
        $stmt = $this->conexion->prepare("SELECT r.Id_Reparacion, 
                                            c.Nombre_Completo, 
    ...
    public function obtenerDispositivosByCliente() {
        $stmt = $this->conexion->prepare("SELECT r.Id_Dispositivo, c.Nombre_Completo, r.Modelo 
    ...
    public function obtenerTecnicoById() {
        $stmt = $this->conexion->prepare("SELECT Id_Tecnico, Nombre_Tecnico FROM datostecnico WHERE Activo = 1");
        $stmt->execute();
        return $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    }
