...
class Cliente {
    ...
    public function insertar($data){
        ...
        $stmt = $this->conexion->prepare("INSERT INTO datoscliente 
    ...
    public function actualizar($data){
        extract($data);
        $nombre_completo = "$nombre $apellido"; // Genera el nombre completo
        $stmt = $this->conexion->prepare("UPDATE datoscliente 
    ...
    public function eliminar($id) {
        $stmt = $this->conexion->prepare("UPDATE datoscliente SET Activo = 0 WHERE Id_Cliente = ?");
    ...
    public function obtenerClientes() {
        $stmt = $this->conexion->prepare("SELECT * FROM datoscliente WHERE Activo = 1");
        $stmt->execute();
    ...
    public function cedulaExiste($cedula, $id = null) {
        if ($id) {
            ...
            $stmt = $this->conexion->prepare("SELECT 1 FROM datoscliente WHERE Cedula = ? AND Id_Cliente != ? AND Activo = 1");
            $stmt->bind_param("si", $cedula, $id);
        } else {
            // Consulta sin excluir ningún ID (útil al crear)
            $stmt = $this->conexion->prepare("SELECT 1 FROM datoscliente WHERE Cedula = ? AND Activo = 1");
