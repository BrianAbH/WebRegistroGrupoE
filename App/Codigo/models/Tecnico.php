...
class Tecnico {
    private $conexion; // Propiedad para almacenar la conexión
    ...
    public function insertar($data){
        ...
        $stmt = $this->conexion->prepare("INSERT INTO datostecnico 
    ...
    public function actualizar($data){
        extract($data);
        $nombre_completo = "$nombre $apellido"; // Se genera el nombre completo del técnico
        $stmt = $this->conexion->prepare("UPDATE datostecnico 
    ...
    public function eliminar($id) {
        $stmt = $this->conexion->prepare("UPDATE datostecnico SET Activo = 0 WHERE Id_Tecnico = ?");
    ...
    public function obtenerTecnico() {
        $stmt = $this->conexion->prepare("SELECT * FROM datostecnico WHERE Activo = 1");
        $stmt->execute();
    ...
    public function cedulaExiste($cedula, $id = null) {
        if ($id) {
            ...
            $stmt = $this->conexion->prepare("SELECT 1 FROM datostecnico WHERE Cedula = ? AND Id_Tecnico != ? AND Activo = 1");
            $stmt->bind_param("si", $cedula, $id);
        } else {
            // Verifica si la cédula ya existe en cualquier técnico activo
            $stmt = $this->conexion->prepare("SELECT 1 FROM datostecnico WHERE Cedula = ? AND Activo = 1");
